...
History = [];
Weight =  ones(dim,1);                                                                                                                   % should start with random between 0 and 1?
History(:,1) = Weight;
P.lambda = lambda;
P.Targets = Targets(:);
% -------------------------------------------------------------------------
...
while  Difference>0.01 & t<=10;
    ...
    for i = 1:N_patterns,
        ...
        for c = 1:length(Uc)
            ...
            % karan printing index here
            %if sum(prob)~=0;prob_1 = prob/sum(prob);else;display('else');[dum,I] = sort(dist);prob(I(2))=1;prob_1=prob;end
